...
# Pattern nome cartella
FOLDER_PATTERN <- "(?i)^([A-Z][A-Z_ -]+?)_(\\d{8})_(\\d+)$"

# Pattern semantici
RX_ABROGATO_TOTALE   <- "(?i)^\\s*\\(\\s*abrogato\\s*\\)|(?i)^\\s*articolo\\s+abrogato"
RX_ABROGATO_PARZIALE <- paste0(
...
CODICI_PRINCIPALI <- list(
  ...
  # D.P.R.
  "decreto.del.presidente.della.repubblica:1952-02-15;328"              = list(nome_comune = "Regolamento esecuzione Cod. Nav.",       codice_breve = "reg. cod. nav."),
  ...
  "decreto.del.presidente.della.repubblica:1988-09-22;447"              = list(nome_comune = "Approvazione c.p.p.",                    codice_breve = "c.p.p. 1988"),
  "decreto.del.presidente.della.repubblica:1992-12-16;495"              = list(nome_comune = "Regolamento esecuzione c.d.s.",          codice_breve = "reg. c.d.s."),
  "decreto.del.presidente.della.repubblica:2010-10-05;207"              = list(nome_comune = "Regolamento esecuzione contratti pubbl.", codice_breve = "reg. contr. pubbl."),
...
#' Valida una data YYYYMMDD in modo rigoroso.
...
#' Costruisce la timeline cronologica dei file XML.
build_timeline <- function(file_xml, data_originale) {
  ...
  # URN della legge modificante dall'ultimo file
    ...
    urn_lookup <- setNames(
      sapply(hrefs, function(h) {
        ...
        m <- regmatches(h, regexpr(
          "/akn/it/act/([^/]+)/([^/]+)/([0-9]{4}-[0-9]{2}-[0-9]{2})/([^/]+)/!main",
          h
        ))
        ...
        tipo     <- tolower(gsub("_", ".", parts[4]))
        autorita <- tolower(parts[5])
        data_v   <- parts[6]
        numero   <- parts[7]
        sprintf("urn:nir:%s:%s:%s;%s", autorita, tipo, data_v, numero)
      }),
      eids
    )
    
    for (k in seq_len(nrow(tl))) {
      source_id <- paste0("rp", tl$versione_id[k])
      if (source_id %in% names(urn_lookup)) {
        urn <- urn_lookup[[source_id]]
        if (!is.na(urn) && nchar(urn) > 0) {
          tl$urn_legge_modificante[k] <- urn
        }
      }
    }
  }, error = function(e) {
    warning(sprintf("Impossibile estrarre URN modificanti: %s", e$message),
            call. = FALSE)
  })
...
extract_node_id <- function(nodo_xml, testo_incipit) {
  id_strutturale <- xml2::xml_attr(nodo_xml, "eId") %||%
    xml2::xml_attr(nodo_xml, "id")
...
#' Estrae i metadati di un allegato.
...
#' Classifica il tipo di intervento normativo.
...
#' Estrae il testo di un nodo AKN.
extract_testo_nodo <- function(nodo_xml) {
  nome_tag <- xml2::xml_name(nodo_xml)
  
  if (nome_tag == "doc") {
    nodi_testo <- xml2::xml_find_all(
      nodo_xml,
      "./mainBody/paragraph/content/p | ./mainBody/paragraph/content | ./mainBody/p"
    )
  } else {
    nodi_testo <- xml2::xml_find_all(
      nodo_xml,
      paste0(
        ...
        ".//content[not(ancestor::section)][not(ancestor::note)]",
        "[not(ancestor::passiveModifications)][not(ancestor::activeModifications)]"
      )
    )
  }
...
# Compatibilita' con codice esistente.
estrai_modificato_da <- function(doc, versione_global_id = NA_character_) {
  list()
}

#' [DEPRECATA] mantenuta per compatibilita'
  ...
  lifecycle_lookup <- tryCatch({
    ...
    setNames(
      as.list(suppressWarnings(
        as.Date(xml2::xml_attr(eventi, "date"))
      )),
...
#' Estrae le modifiche ricevute dall'atto e la relativa data GU.
estrai_passive_modifications <- function(doc) {
  ...
  if (length(nodi_pass) == 0) {
    return(list(
      tipo_modifica_ricevuta  = NA_character_,
      data_gu_modifica        = NA_integer_,
      testo_modifica_ricevuta = NA_character_
    ))
  }
  
  for (mod in nodi_pass) {
